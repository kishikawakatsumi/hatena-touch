...
@interface DiaryFeedParser(LibXMLParserMethods)
...
static NSUInteger kUpdatedElementNameLength = 8;
static const char *kPublishedElementName = "published";
static NSUInteger kPublishedElementNameLength = 8;
static const char *kContentElementName = "content";
static NSUInteger kContentElementNameLength = 8;
...
defaultAttributeCount:(int)defaultAttributeCount attributes:(xmlSAX2Attributes *)attributes {    
    if (strncmp((char *)localname, kEntryElementName, kEntryElementNameLength) == 0) {
        ...
        [[diaries objectForKey:@"entries"] addObject:currentEntry];
        return;
    }
    
    if (strncmp((char *)localname, kTitleElementName, kTitleElementNameLength) == 0 ||
        strncmp((char *)localname, kUpdatedElementName, kUpdatedElementNameLength) == 0 ||
        strncmp((char *)localname, kPublishedElementName, kPublishedElementNameLength) == 0 ||
        strncmp((char *)localname, kContentElementName, kContentElementNameLength) == 0) {
    ...
    if (strncmp((char *)localname, kLinkElementName, kLinkElementNameLength) == 0) {        
        ...
        if ([relValue isEqualToString:@"edit"]) {
...
static void startElementSAX(void *ctx, const xmlChar *localname, const xmlChar *prefix,
    ...
    [((DiaryFeedParser *)ctx) elementFound:localname prefix:prefix uri:URI 
...
static void	endElementSAX(void *ctx, const xmlChar *localname, const xmlChar *prefix,
                          const xmlChar *URI) {    
    [((DiaryFeedParser *)ctx) endElement:localname prefix:prefix uri:URI];
}

static void	charactersFoundSAX(void *ctx, const xmlChar *ch, int len) {
    [((DiaryFeedParser *)ctx) charactersFound:ch length:len];
}

static void errorEncounteredSAX(void *ctx, const char *msg, ...) {
    va_list argList;
    va_start(argList, msg);
    [((DiaryFeedParser *)ctx) parsingError:msg, argList];
}

static void endDocumentSAX(void *ctx) {
    [((DiaryFeedParser *)ctx) endDocument];
}
